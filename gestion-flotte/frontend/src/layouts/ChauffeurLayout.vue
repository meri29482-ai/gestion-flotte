<template>
  <div id="chauffeur-layout" class="d-flex flex-column vh-100">
    <!-- Header -->
    <header class="header d-flex align-items-center justify-content-between px-4 shadow-sm">
      <div class="logo d-flex align-items-center">
        ...
        <h1 class="ms-2 text-sonatrach">Gestion Parc - Chauffeur</h1>
      </div>
    ...
    <div class="flex-grow-1 d-flex overflow-hidden">
      ...
      <aside class="sidebar bg-light border-end">
        <ul class="nav flex-column py-3">
          <li class="nav-item">
            <router-link to="/chauffeur/dashboard" class="nav-link" exact-active-class="active">
              <i class="bi bi-speedometer2 me-2"></i> Tableau de bord
            </router-link>
          </li>
          <li class="nav-item">
            <router-link to="/chauffeur/missions" class="nav-link" exact-active-class="active">
              <i class="bi bi-flag me-2"></i> Mes Missions
            </router-link>
          </li>
          <li class="nav-item">
            <router-link to="/chauffeur/historique" class="nav-link" exact-active-class="active">
              <i class="bi bi-clock-history me-2"></i> Historique
            </router-link>
          </li>
          <li class="nav-item">
            <router-link to="/chauffeur/notifications" class="nav-link" exact-active-class="active">
              <i class="bi bi-bell me-2"></i> Notifications 
            </router-link>
          </li>
          <li class="nav-item mt-auto">
            <router-link to="/chauffeur/profil" class="nav-link" exact-active-class="active">
...
export default {
 ...
 methods: {
  ...
  fetch(`http://localhost:3000/api/notifications/${notif.id}/lu`, {
    ...
    if (notif.type === "MISSION" && notif.mission_id) {
      this.$router.push(`/chauffeur/missions`);
    } else {
      if (notif.type === "DOCUMENT" && notif.mission_id) {
      this.$router.push(`/chauffeur/profil`);
    }}

    this.showNotifications = false; // Fermer le menu
  });
},
  ...
  toggleNotifications() {
    ...
    if (this.showNotifications) {
      this.fetchNotifications(); // Recharger en ouvrant
    }
  },
  fetchNotifications() {
    const token = localStorage.getItem("token");
...
marquerCommeLue(id) {
  const token = localStorage.getItem("token");
  fetch(`http://localhost:3000/api/notifications/${id}/lu`, {
    method: "PUT",
    headers: {
      Authorization: `Bearer ${token}`,
    },
  }).then(() => {
    // Supprimer la notif directement de la liste affichée :
    this.notifications = this.notifications.filter((notif) => notif.id !== id);
    this.unreadCount = this.notifications.filter((notif) => !notif.lu).length;
