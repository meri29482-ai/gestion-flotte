<template>
  <div class="sh-page">
    ...
    <header class="sh-header">
      <h1 class="sh-title">📜 Historique des Interventions</h1>
    ...
    <div class="sh-filter">
      <input
        type="text"
        v-model="searchQuery"
        placeholder="🔍 Rechercher (achat, maintenance...)"
        class="sh-input"
      />
    </div>
    ...
    <section class="sh-card">
      <div class="sh-card__header">
        <h2 class="sh-card__title">Interventions validées ou rejetées</h2>
      </div>
      ...
      <div class="sh-table-wrapper">
        <table class="sh-table">
          ...
          <tbody>
            <!-- Boucle sur les interventions -->
            <tr v-for="it in filteredInterventions" :key="it.id">
              ...
              <td>
                <span class="sh-badge" :class="priorityClass(it.priorite)">
                  {{ it.priorite }}
                </span>
              </td>
              <td>
                <span class="sh-badge" :class="statusClass(it.statut)">
                  {{ it.statut }}
                </span>
              </td>
            ...
            <tr v-if="filteredInterventions.length === 0">
              <td colspan="7" class="text-center text-muted">
                ❌ Aucune intervention validée ou rejetée trouvée.
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  name: "HistoriqueInterventions",
  data() {
    return {
      interventions: [],
      searchQuery: "",
      selectedIntervention: null, // ✅ indispensable pour ton modal
    };
  },
  computed: {
    filteredInterventions() {
      return this.interventions
        .filter((it) => {
          ...
          return s.includes("validée") || s.includes("rejetée");
        })
        .filter((it) => {
          const q = this.searchQuery.toLowerCase();
          return (
            it.type_demande?.toLowerCase().includes(q) ||
            it.vehicule?.marque?.toLowerCase().includes(q) ||
            it.vehicule?.modele?.toLowerCase().includes(q)
          );
        });
    },
  },
 ...
 calculateTotalParts(pieces) {
    ...
    statusClass(statut) {
      ...
      return {
        ...
        "sh-badge--status-refused": s.includes("rejetée"),
      };
    },
